...
export const tutorPlexWSS = new WebSocketServer({ noServer: true });
console.log("✅ Poll WebSocket server initialized.");

tutorPlexWSS.on("connection", (ws: CustomWebSocket, req: IncomingMessage) => {
  console.log("🔗 Client connected:", req.url);
  ws.send(JSON.stringify({ message: "Connected to Poll WebSocket Server" }));
  ...
  try {
    ...
    ws.on("message", async (data) => {
      try {
        ...
        await handlePollSocket(ws, message);
      } catch (err) {
...
async function handlePollSocket(ws: CustomWebSocket, message: any) {
  try {
    const { type, tutorPlexId, timerDuration, reaction } = message;
    ...
    switch (type) {
      case "fetch_tutor_event":
        console.log("Handling fetch_tutor_event request");
        // Broadcast the resource to all connected clients
        broadcastToAllClients({
          action: "tutor_plex_data",
          data: resource,
        });
        break;
      case "start_timer":
        if (!tutorPlexId || !timerDuration) {
          sendResponse(ws, null, "Missing tutorPlexId or timerDuration.");
          return;
        }
      ...
      case "submit_reaction":
        ...
        if (!tutorPlexId || !reaction) {
          sendResponse(ws, null, "Missing tutorPlexId or reaction.");
          return;
        }
        ...
        try {
          ...
          // Parse the tutorPlexReactions if it exists and is a string
          let tutorPlexReactions = metaInfo[0].tutorPlexReactions;
          if (tutorPlexReactions && typeof tutorPlexReactions === "string") {
            try {
              tutorPlexReactions = JSON.parse(tutorPlexReactions);
            } catch (error) {
              console.error("Error parsing tutorPlexReactions:", error);
              sendResponse(
                ws,
                null,
                "Invalid tutorPlexReactions format: Expected a valid JSON string."
              );
              return;
            }
          }

          // Ensure tutorPlexReactions is an array
          if (!Array.isArray(tutorPlexReactions)) {
            tutorPlexReactions = [];
          }

          // Push the new reaction object into the tutorPlexReactions array
          tutorPlexReactions.push(reactionObj);

          // Update the tutorPlexReactions field in metaInfo
          metaInfo[0].tutorPlexReactions = JSON.stringify(tutorPlexReactions);
      ...
      case "reset_all_reactions":
        try {
          ...
          // Parse the tutorPlexReactions if it exists and is a string
          let tutorPlexReactions = metaInfo[0].tutorPlexReactions;
          if (typeof tutorPlexReactions === "string") {
            tutorPlexReactions = JSON.parse(tutorPlexReactions);
          }
          ...
          // Reset tutorPlexReactions to an empty array
          tutorPlexReactions = [];
    ...
    console.error("❌ PollSocket Error:", error);
